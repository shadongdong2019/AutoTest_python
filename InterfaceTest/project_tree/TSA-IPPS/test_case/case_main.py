...
sheetid_http = int(option_dict["case_sheetid1"])
sheetid_https = int(option_dict["case_sheetid2"])
start= int(option_dict["case_start_rownum"])
end= int(option_dict["case_end_rownum"])

data_http = TsaParamDict(filename,sheetid_http).deal_param(start=start,end=end)
...
class CaseRun(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.description = ""
        self.url = "http://39.107.66.190:9999/v2/api/confirm/opusConfirm" #确权接口  ipp20timestamp.tsa.cn
    ...
    def test_apply_http(self,data_dict):
        ...
        '''
        self.op_excel = OperationExcel(filename, sheetid_http)
        self.url = option_dict["url1"]
        download_url = option_dict["url2"]
...
if __name__ == "__main__":
    ...
    data_str = datetime.datetime.now().strftime('%Y%m%d')
    report_path = os.path.join("reportpath{}_zs/".format(data_str),report_name)
    path = os.path.join("reportpath{}_zs/".format(data_str))
